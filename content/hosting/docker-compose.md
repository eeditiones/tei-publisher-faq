...
Using a docker compose workflow can be a cheap and viable alternative, at least for smaller editions without much traffic to expect. Our docker compose configuration comes with everything preconfigured and you don't need to install any dependencies by hand. It also simplifies occasional updates and general maintenance. On the downside, it will likely not perform or scale as good as a dedicated server, but for smaller edition projects this may not be a problem.
...
{{% notice note %}}
Unless you are using TEI Publisher 8 or a development build, you can also remote the entire `ner:` section from `docker-compose.yml`, including the `depends_on: - ner` in the line above.
{{% /notice %}}
